...
<body>
        ...
        <div class="container m-0">
            ...
            <div class="col-md-11 text-container" style="max-height: 800px; overflow-y: auto; overflow-x: hidden;">
                ...
                <p class="text-justify">
                    ...
                    <b>1 -></b> Click on the components for each Category of product you want
                    <b>2 -></b> Now click next or on the categories to move forward and choose all options, then after choosing all components, ADD TO CART button shows up. <b>(REMEMBER to click next or back button after choosing all components to get the add to cart button)</b> 
                    <b>3 -></b> You can either remove all and start creating again or move forward with the components you have.
                </p>
    ...
    <script>
        ...
        var componentNames = 
        {
            ...
            2: 'Motherboard',
            3: 'Memory (RAM)',
            4: 'Graphics Card (GPU/VGA)',
            5: 'Solid State Drive (SSD)',
            6: 'Hard Disk Drive (Internal HDD)',
            7: 'Cooling System (CPU Cooler)',
        ...
        // Function to check if all components are selected
        ...
        // Hide the "Add to Cart" button if not all components are selected
        ...
        document.getElementById('addToCart').addEventListener('click', function(event) {
            ...
            // Check if the user's cart is already full
            var xhrCheck = new XMLHttpRequest();
            xhrCheck.onreadystatechange = function() {
                if (xhrCheck.readyState === XMLHttpRequest.DONE) {
                    if (xhrCheck.status === 200) {
                        var response = xhrCheck.responseText;
                        if (response === 'Cart is full') {
                            // Display a message that the cart is full
                            Swal.fire({
                                title: 'Cart is full!',
                                text: 'Your cart already contains items. Please complete the current order or remove items before adding new ones.',
                                icon: 'error',
                                showClass: {
                                ...
                                window.location.href = 'build_pc.php';
                            });
                        } else if (response === 'Cart is empty') {
                            // Iterate through the session storage items and insert the product into the database table
                            for (var key in sessionStorage) {
                                if (key.startsWith('selectedProduct_')) {
                                    // Parse the selected product data from session storage
                                    var selectedProduct = JSON.parse(sessionStorage.getItem(key));
                                    var productId = selectedProduct.productId;
                                    var title = selectedProduct.title;
                                    var discount = selectedProduct.discount;
                                    var userId = selectedProduct.userId;
                                    ...
                                    xhr.onreadystatechange = function() {
                                        if (xhr.readyState === XMLHttpRequest.DONE) {
                                            if (xhr.status === 200) {
                                                ...
                                                console.log('Product inserted into database: ' + title);
                                                Swal.fire({
                                                    title: 'PC Components added to cart!',
                                                    icon: 'success',
                                                    showClass: {
                                                        popup: 'animate__animated animate__shakeX'
                                                    },
                                                ...
                                                console.error('Error inserting product: ' + title);
                                                Swal.fire({
                                                    title: 'Error',
                                                    text: 'Error inserting product: ' + title,
                                                    icon: 'error',
                                                    showClass: {
                                                        popup: 'animate__animated animate__shakeX'
                                                    },
                                    ...
                                    xhr.send('productId=' + productId + '&title=' + title + '&discount=' + discount + '&userId=' + userId);
                                }
                            }
                        ...
                        Swal.fire({
                            title: 'Error',
                            text: 'Error checking cart status.',
                            icon: 'error',
                            showClass: {
                                popup: 'animate__animated animate__shakeX'
                            },
            ...
            xhrCheck.open('POST', 'check_cart_empty.php', true);
            xhrCheck.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhrCheck.send('userId=' + <?php echo $_SESSION['user_id']; ?>);
        });
